...
abstract class AutoRoutine : SequentialCommandGroup(), Source<Command> {
    ...
    override fun invoke() = (sequential {
        ...
        +routine
    }.raceWith(WaitUntilCommand { Robot.emergencyActive })).whenFinished { DriveSubsystem.compressor.start() } as CommandBase
    ...
    protected fun relocalize(position: Pose2d, forward: Boolean, pathMirrored: BooleanSource, isStowed: Boolean = true, isPoked: Boolean = false) = InstantCommand(Runnable {
        ...
        ) + if (forward) (if (isStowed && isPoked) Constants.kForwardIntakePokedStowedToCenter else if(isStowed && !isPoked)
            Constants.kForwardIntakeStowedToCenter else Constants.kForwardIntakeToCenter) else Constants.kBackwardIntakeToCenter
