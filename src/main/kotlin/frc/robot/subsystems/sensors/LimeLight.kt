...
object LimeLight {
    ...
    val targetToTheLeft get() = currentState.skew > (-45).degrees
    ...
    data class State(
        ...
        constructor() : this(false, 0.degrees, 0.degrees, 0.degrees, 0.0, 0.0, 0.second)
    }
    ...
    fun tangentDistance(isHatchTarget: Boolean = true): SIUnit<Meter> {
        // Thanks 1678
        val limeHeight = 42.25.inches - 1.inches + 2.inches
        val targetHeight = if (isHatchTarget) 28.6.inches else 36.inches
        val limelightPitch = (-30).degrees
        val lastPitch = currentState.pitch
        val distance = tan(
                (lastPitch + limelightPitch).radian *
                        (limeHeight - targetHeight).inMeters()
        ).meters
        return distance
    }
    ...
    fun update() {
        ...
        val newState = State(
                ...
                -txEntry.getDouble(0.0).degrees,
                tyEntry.getDouble(0.0).degrees,
                tsEntry.getDouble(0.0).degrees,
                widthEntry.getDouble(0.0),
                heightEntry.getDouble(0.0),
                Timer.getFPGATimestamp().seconds - latencyEntry.getDouble(0.0).milli.second - 11.milli.second
        )
